...
from src.books.schemas import bookModel, singleBook, updateBookModel, BookCreatedModel, BookDetailModel
...
@book_router.get("/user/{user_uid}", response_model=List[bookModel], status_code= status.HTTP_200_OK, dependencies=[role_checker])
...
@book_router.get("/{book_uid}", response_model=BookDetailModel, status_code=status.HTTP_200_OK, dependencies=[role_checker])
async def getBook_by_id(
...
async def create_book(
    book_data:BookCreatedModel,
    session:AsyncSession = Depends(get_session),
    token_details: dict= Depends(access_token_bearer)
    ):
    user_id = uuid.UUID( token_details.get('user')['user_uid']) #type: ignore
...
async def update_by_id(
    book_uid:str, book_to_update: updateBookModel,
    session:AsyncSession=Depends(get_session),
    token_details=Depends(access_token_bearer)
    ):
