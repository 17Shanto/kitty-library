...
async def getBook_by_id(book_uid:str, session: AsyncSession = Depends(get_session)):
...
async def create_book(book_data:BookCreatedModel, session:AsyncSession = Depends(get_session)):
    new_book = await book_service.create_book(book_data,session)
    return new_book
    # raise HTTPException(status_code = status.HTTP_201_CREATED, detail="New Book is added successfully")


@book_router.patch("/{book_uid}", response_model= bookModel, status_code= status.HTTP_200_OK)
async def update_by_id(book_uid:str, book_to_update: updateBookModel, session:AsyncSession=Depends(get_session)):
...
async def delete_by_id(book_uid:str, session: AsyncSession=Depends(get_session)):
